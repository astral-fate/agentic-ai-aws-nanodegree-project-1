...
cat > system_prompt.txt <<'PROMPT_EOF'
...
--- OTHER --------------------------------------------------------
  - One short, warm sentence showing you understood what they asked.
  - Say plainly that this is not something you can help with from this
    chat.
  - Give them the human support line, exactly as written:
    1-800-555-0199, available Monday to Friday.
...
info "writing harness-tests.json (21 cases, all three routes)"
...
TESTS_EOF
cp harness-tests.json flow-tests.json   # the rubric uses the older filename

info "writing scripted_bug_report.py"
cat > scripted_bug_report.py <<'SCRIPTED_EOF'
#!/usr/bin/env python3
...
TICKET_RE = re.compile(r"\b[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\b", re.I)
...
def send(rt, config, session_id, text, transcript):
    ...
    for event in event_stream(response):
        if "contentBlockStart" in event:
            ...
            d = event["contentBlockDelta"].get("delta", {})
            if "text" in d:
                print(d["text"], end="", flush=True)
                buffer.append(d["text"])
        elif "messageStop" in event:
            if buffer:
                texts.append("".join(buffer)); buffer = []
    if buffer:
...
def check(label, ok_, detail=""):
    print(f"  [{'PASS' if ok_ else 'FAIL'}] {label}" + (f" - {detail}" if detail else ""))
    return ok_
...
def main():
    ...
    print("\n" + "=" * 62 + "\nVERIFICATION\n" + "=" * 62)
    results = [
        check("the create_bug_report tool was called", TOOL_NAME in all_tools,
              f"saw {all_tools or 'no tool calls'}"),
        check("it was called exactly once", all_tools.count(TOOL_NAME) == 1,
              f"called {all_tools.count(TOOL_NAME)}x"),
    ]
    match = TICKET_RE.search(last_reply)
    results.append(check("the assistant relayed a ticket ID", match is not None))
    if not match:
    ...
    print(f"\n  ticket: {ticket_id}")
    table = boto3.resource("dynamodb", region_name=config["region"]).Table(config["table_name"])
    ...
    for field, needles in EXPECTED_CONTENT.items():
        ...
        results.append(check(f"{field} reflects what the customer said",
                             all(n in value for n in needles), f"looked for {needles}"))

    print("\n" + "=" * 62)
    if all(results):
        print(f"ALL {len(results)} CHECKS PASSED"); print("=" * 62); return
    print(f"{results.count(False)} of {len(results)} CHECKS FAILED"); print("=" * 62)
    sys.exit(1)
...
cat > run_evaluation.py <<'RUNEVAL_EOF'
#!/usr/bin/env python3
...
def check_dataset(path: Path, model_identifier: str) -> int:
    ...
    for n, line in enumerate(lines, 1):
        try:
            ...
            errors.append(f"line {n}: not valid JSON ({exc})"); continue
        missing = {"prompt", "referenceResponse", "modelResponses"} - set(record)
        if missing:
            errors.append(f"line {n}: missing {sorted(missing)}"); continue
        responses = record["modelResponses"]
        if not isinstance(responses, list) or not responses:
            errors.append(f"line {n}: modelResponses must be a non-empty list"); continue
        got = responses[0].get("modelIdentifier")
        if got != model_identifier:
            errors.append(f"line {n}: modelIdentifier is {got!r} but the job "
                          f"will look for {model_identifier!r}")
...
def main():
    ...
    n = check_dataset(dataset, args.model_identifier)
    print(f"{dataset} looks well-formed ({n} records).")
    ...
    boto3.client("s3", region_name=args.region).upload_file(str(dataset), bucket, key)
    ...
    Path("eval_job.json").write_text(
        json.dumps({"jobArn": job_arn, "jobName": job_name, "bucket": bucket}, indent=2),
        encoding="utf-8")

    if not args.wait:
        print("\nRe-run with --wait to poll until it finishes."); return
    ...
    while True:
        ...
        time.sleep(30)
    print(f"\nDone. Results: s3://{bucket}/results/")
