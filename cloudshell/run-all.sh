...
info "writing cloudformation-tool.yaml"
cat > cloudformation-tool.yaml <<'INLINE_CLOUDFORMATION_TOOL_YAML'
# Bug report tool + AgentCore IAM roles.
#
# The inline Lambda code below mirrors create_bug_report.py — if you change
# one, keep the other in sync.
#
# This template creates everything your chatbot needs on the "classic AWS"
# side:
#   * a DynamoDB table that stores bug tickets
#   * the create_bug_report Lambda function (the tool implementation)
#   * three IAM roles:
#       1. the Lambda's own execution role (logs + PutItem on the table)
#       2. the AgentCore *Gateway* role — the gateway assumes this role to
#          invoke the Lambda on the model's behalf
#       3. the AgentCore *harness execution* role — the managed harness
#          assumes this role to call Bedrock models, write logs/metrics,
#          and invoke the gateway
#
# The AgentCore resources themselves (gateway, gateway target, harness) are
# created afterwards with the setup_gateway.py and create_harness.py scripts,
# which read this stack's outputs automatically.

AWSTemplateFormatVersion: '2010-09-09'
Description: Bug report tool - DynamoDB table, Lambda function, and IAM roles for Amazon Bedrock AgentCore
...
Resources:
  ...
  BugReportsTable:
    ...
    Properties:
      TableName: !Sub ${AWS::StackName}-bug-reports
          ...
          PolicyDocument:
            ...
            Statement:
              - Effect: Allow
                ...
                Resource: !GetAtt BugReportsTable.Arn

  CreateBugReportFunction:
    Type: AWS::Lambda::Function
    Properties:
      FunctionName: !Sub ${AWS::StackName}-create-bug-report
      Runtime: python3.12
      ...
      Environment:
        Variables:
          TABLE_NAME: !Ref BugReportsTable
      Code:
        ZipFile: |
          import json
          import os
          ...
          _PLACEHOLDERS = {
              "not provided", "not provided by customer", "not specified",
              "not given", "not available", "not applicable", "unknown",
              "unspecified", "none", "n/a", "na", "-", "--", "tbd", "no steps",
              "no environment", "not sure", "unsure",
              "customer did not provide", "did not provide",
          }
          ...
          def lambda_handler(event, context):
              ...
              # A blank field is not the only way to file an empty ticket. Told
              # not to invent values, the model's next move is a placeholder - a
              # live run filed a ticket whose stepsToReproduce and environment
              # were both "not provided", on the first turn, without asking
              # anything. Refuse those here rather than trusting the prompt.
  ...
  # The AgentCore Gateway assumes this role to invoke the Lambda function.
  # (With Bedrock Agents Classic this used to be a resource-based Lambda
  # permission for bedrock.amazonaws.com — the gateway model uses an IAM
  # role instead.)
  GatewayRole:
    Type: AWS::IAM::Role
    Properties:
          ...
          PolicyDocument:
            ...
            Statement:
              - Effect: Allow
                ...
                Resource: !GetAtt CreateBugReportFunction.Arn

  # The AgentCore managed harness assumes this role while running your
  # chatbot: calling the Bedrock model, writing logs/metrics/traces, and
  # invoking the gateway to reach your tool.
            ...
            Statement:
              - Sid: InvokeBedrockModels
                ...
                Resource:
                  ...
                  - !Sub arn:aws:bedrock:us-east-1:${AWS::AccountId}:*
                  - !Sub arn:aws:bedrock:*:${AWS::AccountId}:inference-profile/*
              - Sid: ContainerAndTokenAuth
                Effect: Allow
                Action:
              ...
              - Sid: RuntimeLogs
                ...
                Resource:
                  - !Sub arn:aws:logs:us-east-1:${AWS::AccountId}:log-group:/aws/bedrock-agentcore/runtimes/*
                  - !Sub arn:aws:logs:us-east-1:${AWS::AccountId}:log-group:/aws/bedrock-agentcore/runtimes/*:log-stream:*
              ...
              - Sid: WorkloadIdentity
                ...
                Resource:
                  ...
                  - !Sub arn:aws:bedrock-agentcore:us-east-1:${AWS::AccountId}:workload-identity-directory/default/workload-identity/*
              - Sid: Memory
                Effect: Allow
                Action:
                ...
                Resource: !Sub arn:aws:bedrock-agentcore:us-east-1:${AWS::AccountId}:memory/*
              - Sid: CodeInterpreter
                Effect: Allow
                Action:
              ...
              # Without InvokeGateway the harness cannot load your tools —
              # the gateway MCP endpoint returns 403 Forbidden.
              - Sid: InvokeGateway
                Effect: Allow
                Action: bedrock-agentcore:InvokeGateway
                Resource: !Sub arn:aws:bedrock-agentcore:us-east-1:${AWS::AccountId}:gateway/*
              - Sid: Marketplace
                Effect: Allow
                Action:
...
Outputs:
  BugReportsTableName:
    Description: Name of the BugReports DynamoDB table (use it to verify tickets)
    Value: !Ref BugReportsTable

  BugReportsTableArn:
    Description: ARN of the BugReports DynamoDB table
    Value: !GetAtt BugReportsTable.Arn

  LambdaFunctionArn:
    Description: ARN of the create-bug-report Lambda function
    Value: !GetAtt CreateBugReportFunction.Arn
  ...
  GatewayRoleArn:
    Description: ARN of the IAM role the AgentCore Gateway uses to invoke the Lambda
    Value: !GetAtt GatewayRole.Arn

  HarnessExecutionRoleArn:
    Description: ARN of the IAM role the AgentCore managed harness runs with
    Value: !GetAtt HarnessExecutionRole.Arn
INLINE_CLOUDFORMATION_TOOL_YAML
info "writing create_bug_report.py"
cat > create_bug_report.py <<'INLINE_CREATE_BUG_REPORT_PY'
...
def lambda_handler(event, context):
    ...
    # tool result.
    return {"ticketId": ticket_id, "status": "OPEN"}
INLINE_CREATE_BUG_REPORT_PY
chmod +x create_bug_report.py
...
THE GATE - apply this before every single reply in a bug conversation:
  ...
  "not provided", "not specified", "unknown", "N/A", "none" and anything
  like them are NOT values. A field holding one of those is still missing,
  and filing with it is the same failure as inventing a value - it just
  fails more honestly. The tool rejects them, so a ticket filed that way
  does not exist. There is no shortcut here: ask, and keep asking.
...
How to collect them:
  ...
  - Do not accept empty or vague values. "It's broken" is not a
    ...
    sharpen it.
  - If the customer says they do not know, do not give up and do not write a
    placeholder. Ask a smaller question instead: what they remember doing
    just before it happened, or which browser they usually use. A rough
    answer in the customer's own words is a real value; "not provided" is
    not.
...
info "writing setup_flow.py"
cat > setup_flow.py <<'INLINE_SETUP_FLOW_PY'
#!/usr/bin/env python3
...
if __name__ == "__main__":
    main()
INLINE_SETUP_FLOW_PY
chmod +x setup_flow.py
...
# Deploy unconditionally. `cloudformation deploy` creates or updates, and
# skipping it when the stack already exists means template changes never
# reach AWS - which is how a fix to the Lambda validation could sit in the
# repo while the deployed function kept the old behaviour.
info "deploying $TOOL_STACK (creates or updates; a few minutes if changed)..."
DEPLOY_OUT="$(aws cloudformation deploy \
  --template-file cloudformation-tool.yaml \
  --stack-name "$TOOL_STACK" \
  --capabilities CAPABILITY_NAMED_IAM \
  --region "$REGION" 2>&1)" && DEPLOY_RC=0 || DEPLOY_RC=$?

if [ "$DEPLOY_RC" -eq 0 ]; then
  ok "$TOOL_STACK up to date"
elif printf %s "$DEPLOY_OUT" | grep -qi "No changes to deploy"; then
  # Older CLI versions exit non-zero when there is nothing to change.
  ok "$TOOL_STACK already matches the template"
else
  printf %s "$DEPLOY_OUT" >&2; echo
  die "Deploying $TOOL_STACK failed."
fi
...
# ============================================================== FLOW =======
step "Bedrock Flow — the diagram the rubric asks to see"

# The chatbot runs on the AgentCore harness, where routing lives in the
# system prompt and there is no canvas. The rubric was written for Bedrock
# Flows and asks for a flow diagram, a classifier prompt node and Condition
# node expressions - artefacts that only exist if there is a real Flow. This
# builds one with the same three categories and the same routing rules, so
# the console has something to render.
python setup_flow.py || warn "flow setup failed - it is evidence, not a dependency"
if [ -n "$(cfg_get flow_id)" ]; then
  ok "flow $(cfg_get flow_name) ($(cfg_get flow_id))"
  info "$(cfg_get flow_console_url)"
fi
